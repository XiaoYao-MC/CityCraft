...
public class TrafficLightsLeft1BlockEntity extends BlockEntity {

    public TrafficLightsLeft1BlockEntity(BlockPos pos, BlockState state) {
        super(TRAFFIC_LIGHTS_LEFT_1_ENTITY, pos, state);
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state) {
        ...
        switch(dir) {
            case NORTH:
            case SOUTH:
                if (TIME > 0 & TIME <= 40 & (state.get(TRAFFIC_LIGHTS_COLOUR) != 0)) {
                    world.setBlockState(pos, state.with(TRAFFIC_LIGHTS_COLOUR, 3));
                    break;
                }
                if (TIME > 40 & TIME < 57 & (state.get(TRAFFIC_LIGHTS_COLOUR) != 0)) {
                ...
                if (TIME > 60 & (state.get(TRAFFIC_LIGHTS_COLOUR) != 0)) {
                    world.setBlockState(pos, state.with(TRAFFIC_LIGHTS_COLOUR, 3));
                    break;
                }
            case EAST:
            case WEST:
                if (TIME > 0 & TIME < 100 & (state.get(TRAFFIC_LIGHTS_COLOUR) != 0)) {
                    world.setBlockState(pos, state.with(TRAFFIC_LIGHTS_COLOUR, 3));
                    break;
                }
                if (TIME >= 100 & TIME <= 117 & (state.get(TRAFFIC_LIGHTS_COLOUR) != 0)) {
                    world.setBlockState(pos, state.with(TRAFFIC_LIGHTS_COLOUR, 1));
                    break;
                }
